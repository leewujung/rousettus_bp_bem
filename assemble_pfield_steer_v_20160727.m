...
z_loc = [-0.005:0.002:0.007]';
y_loc = ones(length(z_loc),1)*0.004;
...
t_name = sprintf('%02dkHz_x%03d_y%03d',...
    freq/1e3,tongue_loc_all(1,1)*1e3,tongue_loc_all(1,2)*1e3);
title_text = sprintf('%dkHz, tongue loc: varying z, x=%dmm, y=%dmm',...
    freq/1e3,tongue_loc_all(1,1)*1e3,tongue_loc_all(1,2)*1e3);
...
colorbar('Ticks',linspace(0+1/num_tongue/2,1-1/num_tongue/2,num_tongue),...
    'TickLabels',{num2str(tongue_loc_all(:,3)*1e3)},'location','southoutside');
tightmap
