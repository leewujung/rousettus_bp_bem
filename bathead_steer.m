...
fp_path = ['C:\Users\',usrn,'\Dropbox\0_ANALYSIS\bp_processing\20160308_bem3D_bullethead_0.1_fp'];
fname_pre = 'head_src';
...
for iF=1:length(freq_all)
    for iN=1:length(nn_all)
        ...
        % steered pfield
        pfield(:,iN) = p_fp*phase_delay;
    end
    
    pfield_tot = sum(pfield,2);
    
    pfield_tot = reshape(pfield_tot,size(theta_fp));
...
%     contourfm(theta_fp,phi_fp,pfield_tot_dB,0:-3:-30);
    surfm(theta_fp,phi_fp,pfield_tot_dB);
    colorbar
