...
save_path = ['C:\Users\',usrn,'\Dropbox\0_ANALYSIS\bp_processing\20160319_bem3D_ellipsehead0.1_surf_partest'];
% save_path = 'F:\Dropbox\0_ANALYSIS\bp_processing\20160319_bem3D_ellipsehead0.1_surf';
save_fpre = 'ellipsehead';
...
[nodes,elements,elementsQUAD]=readgeomGMSH(['ellipsehead_0.1.msh']); % Load mesh from GMSH file
...
parfor iF=1:length(freq_all)
    ...
    save_fname1 = sprintf('%s_freq%dkHz_bem_mtx.mat',save_fpre,freq_all(iF)/1e3);
    save(fullfile(save_path,save_fname1),'param','shape','bem');
...
%     [theta,phi] = meshgrid(-180:5:180,80:10:100);
    ...
    save_fname2 = sprintf('%s_freq%dkHz_fp_mtx.mat',save_fpre,freq_all(iF)/1e3);
    save(fullfile(save_path,save_fname2),'param','shape','bem_fp');
    
    % Velocity
    nn_all  = [1843, 750, 1287, 682, 1414, 762, 1454, 1009, 1439, 515,...  % right side
               2591, 2703, 2459, 2977, 2487, 3057, 2605, 3304, 2554, 3051];  % left side
