...
tongue_loc_all = [-0.02,0.006,0.005];
...
%     set(ll,'fontsize',12);
    ...
    for iF=1:num_freq
        ...
        figure(fig_mf)
        plot(-c3db_x,c3db_y,'linewidth',3,'color',colorset(iF,:));
    end
...
%     gridm('gcolor',190*ones(1,3)/255,'glinestyle','-');
    ...
    if save_plot_opt
        save_mf_name = sprintf('%s_mf_cntr_tongue_%02d_%02d_%02d',...
            script_name,tongue_loc(:,1)*1e3,tongue_loc(:,2)*1e3,tongue_loc(:,3)*1e3);
        save_mf_name = regexprep(save_mf_name,'-','m');
        saveas(fig_mf,fullfile(save_path,[save_mf_name,'.fig']),'fig');
        ...
        saveas(fig_all,fullfile(save_path,[save_all_name,'.fig']),'fig');
