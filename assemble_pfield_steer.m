...
x_loc = ones(length(y_loc),1)*-0.001;
z_loc = ones(length(y_loc),1)*0.003;
...
t_name = sprintf('x%03d_z%03d',...
    tongue_loc_all(1,1)*1e3,tongue_loc_all(1,3)*1e3);
title_text = sprintf('Tongue loc: varying y, x=%dmm, y=%dmm',...
    tongue_loc_all(1,1)*1e3,tongue_loc_all(1,3)*1e3);
...
for iT=1:num_tongue
    ...
    plot_model_shape_src(subplot(3,num_tongue,iT+num_tongue),...  % shape, horizontal
        h_outline,gap_loc,tongue_loc,h_axlim,'h');
    plot_model_shape_src(subplot(3,num_tongue,iT+num_tongue*2),...  % shape, vertical
        v_outline,gap_loc,tongue_loc,v_axlim,'v');
