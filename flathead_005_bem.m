...
save_path = ['C:\Users\',usrn,'\Dropbox\0_ANALYSIS\bp_processing\20160314_bem3D_flathead0.05'];
% save_path = 'F:\Dropbox\0_ANALYSIS\bp_processing\20160314_bem3D_flathead0.05';
save_fpre = 'flathead';
...
[nodes,elements,elementsQUAD]=readgeomGMSH(['flathead_0.05.msh']); % Load mesh from GMSH file
nodes = nodes(:,[3,1,2]);       % rearrange to get head pointing +z
...
for iF=1:length(freq_all)
    ...
    save(fullfile(save_path,save_fname),'param','shape','bem');
...
%     [theta,phi] = meshgrid(-180:5:180,80:10:100);
    [theta,phi] = meshgrid(-180:2:180,90);
    ...
    % Velocity
    nn_all  = [3099, 3549, 1089, 3314, 2474, 12704, 12381, 15186, 14475, 15116];
    ...
    for iN=1:length(nn_all)
        ...
        save_fname = sprintf('%s_nn%05d_freq%dkHz.mat%',save_fpre,nn,freq_all(iF)/1e3);
