...
%     folder = ['20160314_bem3D_flathead',num2str(mesh_all(iM))];
    folder = '20160315_bem3D_flathead0.1_surf';
...
%         else
            fname = sprintf('%s_nn%04d_freq%dkHz.mat',fname_pre,nn_all(iN),freq/1e3);
%         end
...
%         hold on
        ...
        surfm(BEM.bem_fp.theta,BEM.bem_fp.phi-90,reshape(pp,size(BEM.bem_fp.phi)));
    end
% end